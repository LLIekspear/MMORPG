...
public class RaceChatCommand implements CommandExecutor, TabCompleter {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
		...
		if(playerRace!=null&&!playerRace.getRaceName().equalsIgnoreCase("none")&&!playerRace.getRaceName().equalsIgnoreCase("banished")) {
			if(playerRace.isRaceAdmin(p)||playerRace.isRaceAdviser(p)) {
				...
				playerRace.sendMessageToRaceResidents(ChatColor.GOLD+"[Раса] "+p.getName()+": "+text);
			} else {
				p.sendMessage(ChatColor.RED+"Данным чатом имеют право пользоваться только советники и лидер расы!");
			}
		} else {
