...
public class PlayerEventRacePromoteLeader implements PlayerEvent {

	private OfflinePlayer resident;
	
	public PlayerEventRacePromoteLeader(OfflinePlayer resident) {
	...
	public boolean execute(Player p) {
		try {
			...
			if(playerRace!=null&&!playerRace.getRaceName().equalsIgnoreCase("none")&&!playerRace.getRaceName().equalsIgnoreCase("banished")) {
				playerRace.promoteToLeader(p, resident);
				RaceMenu.open(p);
			} else {
				p.closeInventory();
			}
			...
			e.printStackTrace();
			p.sendMessage(ChatColor.RED+"Возникла ошибка при попытке повысить игрока до лидера расы!");
			p.closeInventory();
			return false;
		}
	}
