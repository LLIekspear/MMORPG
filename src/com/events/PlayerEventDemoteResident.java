...
public class PlayerEventDemoteResident implements PlayerEvent {

	private OfflinePlayer resident;
	
	public PlayerEventDemoteResident(OfflinePlayer resident) {
	...
	public boolean execute(Player p) {
		try {
			...
			if(playerRace!=null&&!playerRace.getRaceName().equalsIgnoreCase("none")&&!playerRace.getRaceName().equalsIgnoreCase("banished")) {
				playerRace.demoteToResident(p, resident);
				RaceMenu.open(p);
			} else {
				p.closeInventory();
			}
